...
class AICodingAgent {
  ...
  private claudeService: ClaudeAnthropicSDK | ClaudeCodeService | ClaudeCodeSDKService;
  ...
  setupRoutes(): void {
    ...
    this.app.get('/legacy/test', (req, res) => {
      ...
      res.json({ message: 'Legacy route works - API only mode', timestamp: new Date().toISOString() });
    });

    // Authentication routes
    // Note: /login route is now handled by Angular Router in the SPA
    ...
    this.app.get('/api', (req, res) => {
      res.json({
        ...
        version: '1.0.0',
        description: 'API-only backend for the AI Coding Agent with Claude integration',
    ...
    this.app.get('*', (req, res) => {
      ...
      res.status(404).json({ 
        ...
        message: 'This is an API-only backend. Please use the Angular frontend at http://localhost:4201',
        path: req.path,
        availableRoutes: [
          '/api/*',
          '/auth/*',
  ...
  async start(): Promise<void> {
    ...
    this.app.listen(this.port, () => {
      console.log(`🚀 AI Coding Agent API server listening on port ${this.port}`);
      console.log(`� API Base URL: http://localhost:${this.port}`);
      console.log(`📋 Angular Frontend: http://localhost:4201`);
      console.log(`💡 This backend now serves APIs only - use the Angular frontend for the UI`);
    });
  }
}
